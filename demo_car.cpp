...
void CCar::Process(int gamepadNumber)
{
	...
	vec2 sThumbL = vec2(0,0); // input2(GAMEPAD_LTHUMB);
	vec2 sThumbR = vec2(0,0); // input2(GAMEPAD_RTHUMB);
	float bLeftTrigger =  input(KEY_LEFT);
	float bRightTrigger = input(KEY_RIGHT);
